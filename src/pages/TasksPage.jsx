...
export default function TasksPage() {
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-[#DCDCDC] rounded-[2px] px-4 py-4 flex flex-wrap gap-3">
        <input placeholder="Поиск..." className="h-[36px] border border-[#DCDCDC] rounded-[2px] px-3 text-sm flex-1 min-w-[200px] outline-none focus:border-[#2E8B7B]" />
        <select className="h-[36px] border border-[#DCDCDC] rounded-[2px] px-2 text-sm"><option>Все типы</option></select>
        <select className="h-[36px] border border-[#DCDCDC] rounded-[2px] px-2 text-sm"><option>Все компании</option></select>
        <select className="h-[36px] border border-[#DCDCDC] rounded-[2px] px-2 text-sm"><option>Все статусы</option></select>
        <button className="h-[32px] px-4 border border-[#DCDCDC] rounded-[2px] text-sm hover:bg-gray-50">Сбросить</button>
      </div>

      <div className="bg-white border border-[#DCDCDC] rounded-[2px] overflow-hidden">
            ...
            <tbody>
              {tasks.map((t) => (
                <tr key={t.id} className="border-b border-[#ECECEC] hover:bg-[#F9F9F9]">
                  ...
                  <td className="px-4 py-2 text-[#F39A32]">{t.expires}</td>
                  <td className="px-4 py-2">
                    <span className={`px-2 py-0.5 rounded-full text-xs ${statusStyles[t.status]}`}>{t.status}</span>
                  </td>
                  <td className="px-4 py-2">
                    <button className="h-[32px] px-3 bg-[#256F63] text-white rounded-[2px] text-xs transition">
                      Загрузить
                    </button>
                    <button className="ml-2 h-[32px] px-3 border border-[#DCDCDC] rounded-[2px] text-xs hover:bg-gray-50 transition">
                      Карточка
                    </button>
                  </td>
                </tr>
